...
<Banner disableAnnouncer>
  <BannerContent>
    Note: ensure the CSS <code>height</code> property for the grid element has been set
    appropriately to see grid on screen. You can do so by passing a custom class
    name, using a style prop, or by targeting the default <code>.saltGrid</code> class
    name.
  </BannerContent>
</Banner>
